...
* `resolveCacheDir` and `resolveDbPath` (in `cli/lib/resolve-cache-dir.ts`) now accept an optional `projectDir` parameter that anchors path resolution. The default of `""` preserves the prior cwd-relative behavior for direct CLI invocations.
