	...
	Effect.gen(function* () {
		...
		return {
			...
			readBaselines: (cacheDir) =>
				Effect.gen(function* () {
					const content = yield* fs.readFileString(`${cacheDir}/baselines.json`);
					const data = yield* Effect.try({
						try: () => Schema.decodeUnknownSync(CoverageBaselines)(JSON.parse(content)),
						catch: (e) =>
							new CacheError({
								operation: "read",
					...
					Effect.catchTag("SystemError", (error) =>
						error.reason === "NotFound"
							? Effect.succeed(Option.none<CoverageBaselines>())
							: Effect.fail(
									new CacheError({
										operation: "read",
										path: `${cacheDir}/baselines.json`,
			...
			readTrends: (cacheDir, projectName) =>
				Effect.gen(function* () {
					...
					const content = yield* fs.readFileString(filePath);
					const data = yield* Effect.try({
						try: () => Schema.decodeUnknownSync(TrendRecord)(JSON.parse(content)),
						catch: (e) =>
							new CacheError({
								operation: "read",
								path: filePath,
					...
					Effect.catchTag("SystemError", (error) =>
						error.reason === "NotFound"
							? Effect.succeed(Option.none<TrendRecord>())
							: Effect.fail(
									new CacheError({
										operation: "read",
