 ...
 * the terminal can also run it on demand with --since or --cc-session-id.
 *
...
const sinceOption = Options.optional(Options.integer("since"));
	...
	(opts) =>
		Effect.gen(function* () {
			const md = yield* formatWrapupEffect({
				...(opts.since._tag === "Some" && { sessionId: opts.since.value }),
