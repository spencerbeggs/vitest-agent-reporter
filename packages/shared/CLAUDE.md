...
src/
  ...
  schemas/            -- Effect Schema definitions (Common, AgentReport,
    ...
    turns/            -- 2.0.0-α discriminated TurnPayload union over
  ...
  migrations/         -- 0001_initial.ts (1.x 25-table schema),
                         0002_comprehensive.ts (2.0.0-α
                         drop-and-recreate -- 40 tables + corrected
                         notes_fts BEFORE/AFTER UPDATE triggers; the
                         LAST drop-and-recreate per Decision D9)
  ...
  utils/              -- pure utilities: resolve-data-path,
                         ...
                         2.0.0-α adds: function-boundary (acorn AST
                         walk), failure-signature (sha256 hash for
                         stable failure identity),
                         validate-phase-transition (TDD D2
...
| File | Purpose |
| ---- | ------- |
| `services/DataStore.ts` + `layers/DataStoreLive.ts` | All SQLite writes; defines `SettingsInput`, `SessionInput`, `TurnInput`, and other write input types |
| `services/DataReader.ts` + `layers/DataReaderLive.ts` | All SQLite reads; assembles domain types via `sql/assemblers.ts`. 2.0.0-α adds `getSessionById`, `searchTurns`, `computeAcceptanceMetrics` |
...
| `migrations/0002_comprehensive.ts` | 2.0.0-α drop-and-recreate. 40 tables: 25 1.x tables (recreated, with `signature_hash` added to `test_errors` and source-map columns added to `stack_frames`) plus 15 new tables (`sessions`, `turns`, `tool_invocations`, `file_edits`, `hypotheses`, `commits`, `run_changed_files`, `run_triggers`, `build_artifacts`, `tdd_sessions`, `tdd_session_behaviors`, `tdd_phases`, `tdd_artifacts`, `failure_signatures`, `hook_executions`). Per Decision D9, this is the **last** drop-and-recreate; future migrations are ALTER-only |
| `utils/function-boundary.ts` (2.0.0-α) | `findFunctionBoundary(source, line)` parses via `acorn` and returns the smallest enclosing function's start line + name |
| `utils/failure-signature.ts` (2.0.0-α) | `computeFailureSignature` produces a 16-char sha256 from `error_name`, normalized assertion shape, top-frame function name, and function-boundary line. See Decision D10 |
| `utils/validate-phase-transition.ts` (2.0.0-α) | Pure validator for TDD phase transitions; returns acceptance or a typed `DenialReason` + remediation. See Decision D11 |

## Conventions

- **No internal deps.** Never import from `vitest-agent-reporter`,
  `vitest-agent-reporter-cli`, or `vitest-agent-reporter-mcp`. Keeps
...
- **Three external Effect-ecosystem deps unique to this package:**
  ...
  from here instead. 2.0.0-α adds `acorn ^8.16.0` (with
  `@types/acorn ^6.0.4` devDep) for `function-boundary.ts`'s AST walk
  -- also unique to shared.
...
- Renaming a public export: search both runtime packages
  ...
  committing.
- Adding a new turn payload type (2.0.0-α): add the `Schema.Struct` to
