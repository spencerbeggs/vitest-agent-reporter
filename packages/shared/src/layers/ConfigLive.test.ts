...
beforeEach(() => {
	workspaceDir = mkdtempSync(join(tmpdir(), "vitest-agent-reporter-config-"));
...
const loadConfig = (projectDir: string) =>
	Effect.runPromise(
		Effect.gen(function* () {
			...
			return yield* config.loadOrDefault(new VitestAgentReporterConfig({}));
...
describe("ConfigLive", () => {
	...
	it("loads cacheDir override from a workspace-root config file", async () => {
		writeFileSync(join(workspaceDir, "vitest-agent-reporter.config.toml"), 'cacheDir = "/tmp/custom"\n');
		const result = await loadConfig(workspaceDir);
		expect(result.cacheDir).toBe("/tmp/custom");
	});

	it("loads projectKey override from a workspace-root config file", async () => {
		writeFileSync(join(workspaceDir, "vitest-agent-reporter.config.toml"), 'projectKey = "my-app-personal"\n');
		const result = await loadConfig(workspaceDir);
	...
	it("loads both fields from a single config file", async () => {
		writeFileSync(
			join(workspaceDir, "vitest-agent-reporter.config.toml"),
			'cacheDir = "/tmp/custom"\nprojectKey = "my-app"\n',
		);
		const result = await loadConfig(workspaceDir);
		expect(result.cacheDir).toBe("/tmp/custom");
