...
beforeEach(() => {
	dataRoot = mkdtempSync(join(tmpdir(), "vitest-agent-reporter-data-"));
});
...
const fakeConfigFile = (config: VitestAgentReporterConfig) => {
	const service: VitestAgentReporterConfigFileService = {
	...
	return Layer.succeed(VitestAgentReporterConfigFile, service);
};
...
const run = (
	...
	config: VitestAgentReporterConfig,
...
describe("resolveDataPath", () => {
	it("uses programmatic options.cacheDir over everything else", async () => {
		const override = mkdtempSync(join(tmpdir(), "vitest-agent-reporter-override-"));
		const result = await run(
			"/repo",
			{ cacheDir: override },
			new VitestAgentReporterConfig({ cacheDir: "/should-not-win", projectKey: "ignored" }),
		);
	...
	it("falls back to config file cacheDir when no programmatic override", async () => {
		const override = mkdtempSync(join(tmpdir(), "vitest-agent-reporter-config-cache-"));
		const result = await run("/repo", {}, new VitestAgentReporterConfig({ cacheDir: override, projectKey: "ignored" }));
		expect(result).toBe(join(override, "data.db"));
		rmSync(override, { recursive: true, force: true });
	});
	...
	it("uses config file projectKey under XDG data when no cacheDir", async () => {
		const result = await run(
			"/repo",
			{},
			new VitestAgentReporterConfig({ projectKey: "my-app-personal" }),
	...
	it("normalizes a config file projectKey before using it", async () => {
		const result = await run("/repo", {}, new VitestAgentReporterConfig({ projectKey: "@org/custom" }));
	...
	it("falls back to workspace name under XDG data when no overrides", async () => {
		const result = await run("/repo", {}, new VitestAgentReporterConfig({}), "@org/pkg");
		expect(result).toBe(join(dataRoot, "@org__pkg", "data.db"));
	});

	it("ensures the parent directory exists for the database", async () => {
		const result = await run("/repo", {}, new VitestAgentReporterConfig({}), "my-app");
		expect(existsSync(dirname(result))).toBe(true);
	});

	it("returns the same path for two projectDirs sharing a workspace name", async () => {
		const a = await run("/code/my-app", {}, new VitestAgentReporterConfig({}), "my-app");
		const b = await run("/worktrees/my-app-branch", {}, new VitestAgentReporterConfig({}), "my-app");
		expect(a).toBe(b);
	});

	it("fails loudly when no overrides and no workspace name", async () => {
		await expect(run("/repo", {}, new VitestAgentReporterConfig({}), null)).rejects.toThrow(/Workspace root not found/);
	});
});
