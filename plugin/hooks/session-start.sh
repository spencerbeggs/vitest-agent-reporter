...
**This conversation's session id is \`$cc_session_id\`.** As your first MCP tool call, run \`mcp__vitest-agent-reporter__set_current_session_id({ id: \"$cc_session_id\" })\` so the MCP server's session-aware tools default to this id. (If the server was already seeded by the plugin manifest, the call is a harmless no-op.)
