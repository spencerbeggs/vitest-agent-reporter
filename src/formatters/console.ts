...
import type { AgentReport, FileCoverageReport } from "../types.js";
import type { AnsiOptions } from "../utils.js";
 ...
 *
 * @privateRemarks
 * The `"full"` mode is accepted but currently behaves identically to
 * `"failures"` because `AgentReport.failed` only contains modules with
 * failures. Rendering all modules (including passing) requires the report
 * to carry the full module list, which is planned for Phase 2.
 *
