...
function formatProjectBody(report: AgentReport): string[] {
	...
	if (report.coverage) {
		...
		if (cov.lowCoverage.length > 0) {
			lines.push("> [!WARNING]");
			lines.push(`> ${cov.lowCoverage.length} file(s) below coverage threshold (${cov.threshold}%):`);
			lines.push(">");
 ...
 *
 * @see {@link https://docs.github.com/en/actions/writing-workflows/choosing-what-your-workflow-does/workflow-commands-for-github-actions#adding-a-job-summary | GitHub Actions job summaries}
 * @internal
 */
