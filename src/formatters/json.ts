...
import type { AgentReport, AgentReporterOptions, ModuleReport, ReportError, TestReport } from "../types.js";
...
function mapErrors(
	...
	return errors.map((e) => {
		const err: ReportError = { message: e.message };
		if ("diff" in e && e.diff != null) err.diff = e.diff;
		if (e.stacks && e.stacks.length > 0) err.stack = e.stacks.join("\n");
		return err;
	});
}
	...
	for (const testModule of testModules) {
		...
		for (const testCase of allTests) {
			...
			if (shouldInclude) {
				...
				if (diag.flaky) testReport.flaky = true;
				if (diag.slow) testReport.slow = true;
				const errors = mapErrors(result.errors);
				if (errors) testReport.errors = errors;
		...
		if (moduleHasFailure) {
			...
			if (moduleErrors) moduleReport.errors = moduleErrors;
